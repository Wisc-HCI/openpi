...
FORMAL_OUTPUT_DIR="${FORMAL_OUTPUT_DIR:-artifacts/goal_cream_bowl_steering_formal_100}"
...
# Formal statistics use JSON metadata and compressed NPZ trajectories. Videos
# are opt-in because saving hundreds of MP4s adds disk and encoding overhead;
# the six one-shot smoke videos remain the visual sanity check.
FORMAL_SAVE_VIDEO="${FORMAL_SAVE_VIDEO:-false}"
...
start_server "--env LIBERO --negative-prompt-file examples/libero/prompts/goal_bowl_stove.txt --guidance-scale $STEERING_SCALE --guidance-decay $STEERING_DECAY"
run_condition time_decay cream_cheese

start_server "--env LIBERO --negative-prompt-file examples/libero/prompts/goal_cream_cheese.txt --guidance-scale $STEERING_SCALE --guidance-decay $STEERING_DECAY"
run_condition time_decay bowl_stove
...
start_server "--env LIBERO --negative-prompt-file examples/libero/prompts/goal_bowl_stove.txt $BELIEF_ARGS"
run_condition belief cream_cheese

start_server "--env LIBERO --negative-prompt-file examples/libero/prompts/goal_cream_cheese.txt $BELIEF_ARGS"
run_condition belief bowl_stove
...
UV_CACHE_DIR=/tmp/openpi-uv-cache uv run python examples/libero/analyze_black_bowl_steering.py \
    ...
    --episode-prefix goal_cream_bowl \
    --expected-targets cream_cheese,bowl_stove \
    --report-title "Goal cream-cheese / bowl-stove steering formal evaluation" \
    --strict
